...
Not implemented or verified: raw GPIO handling, metastability protection, a periodic sample-enable generator, trigger detection, pre-trigger history, PC transfer, board pin assignments, Quartus synthesis, inferred memory resources, timing closure, or physical board operation. FPGA-only integration remains the next hardware stage; HPS/Linux is optional future work.
